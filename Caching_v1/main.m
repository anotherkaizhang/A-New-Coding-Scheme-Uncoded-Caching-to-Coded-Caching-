...
if N == 0
    partitionTable = 0; %partitions of 0
elseif N == 1
    partitionTable = 1; %partitions of 1
elseif N == 2
    partitionTable = [2,0;1,1]; %partitions of 2
elseif N >= 3
    tempmat = zeros(table(N+1),min(N,maxnum)); count = 1;
    tempmat(1,1) = N;
    ...
    partitionTable = sort(tempmat,2,'descend');
